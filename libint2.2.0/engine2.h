/*
 *  Modified by Soohaeng Yoo Willow (May 5 2016)
 *  --- Support the gradients of 2-electron integrals.
 *
...
#define BOOST_PP_NBODY_DERIV_ORDER_LIST BOOST_PP_TUPLE_TO_LIST( BOOST_PP_NBODY_DERIV_ORDER_TUPLE )
      ...
      template <Operator oper, BraKet braket, size_t deriv_order>
      inline void compute2_deriv (const Shell& s1,
				  const Shell& s2,
				  const Shell& s3,
				  const Shell& s4);

      template <Operator oper, BraKet braket, size_t deriv_order>
      inline const real_t* compute2_deriv_target (const Shell& s1,
						  const Shell& s2,
						  const Shell& s3,
						  const Shell& s4,
						  const int id);
...
#  endif
  ...
  template <Operator oper, BraKet braket, size_t deriv_order>
  inline void
  Engine::compute2_deriv (const libint2::Shell& tbra1,
			  const libint2::Shell& tbra2,
			  const libint2::Shell& tket1,
			  const libint2::Shell& tket2) {
...
#  endif
    ...
    LIBINT2_PREFIXED_NAME(libint2_build_eri1)[bra1.contr[0].l][bra2.contr[0].l][ket1.contr[0].l][ket2.contr[0].l](&primdata_[0]);
    
  }

  template <Operator oper, BraKet braket, size_t deriv_order>
  inline const real_t*
  Engine::compute2_deriv_target (const libint2::Shell& tbra1,
				 const libint2::Shell& tbra2,
				 const libint2::Shell& tket1,
				 const libint2::Shell& tket2,
				 const int id) {
    ...
    real_t* result = primdata_[0].targets[id];
